...
std::map< int, std::set<int> > graph::REFINE( std::map< int , std::set<int> > &A , std::vector< int > &Vs ){
 std::map< int , std::set<int> > B = A;
 std::map< int , std::set<int> > S;
 for( auto it : B )
  S[ B.size() - 1 - it.first ] = it.second;
...
int graph::AbsAndSet( int u, std::set< int > &T , std::vector< int > &Vs ){
 int res = 0;
 //for( int i = 0 ; i < n ; i++ ) 
 for( int i : Vs ) 
  if( is_edge( u , i ) ) {
